...
{
    std::unique_ptr<model::FigureInterface> c(
        std::make_unique<model::Circle>(6.5, 8.0, 5.0));
    _objects.push_back(std::move(c));
}

void Controller::AddSquare()
{
    std::unique_ptr<model::FigureInterface> s(
        std::make_unique<model::Square>(Point { 40.0, 3.0 },
            Point { 40.0, 16.0 },
            Point { 56.0, 16.0 },
            Point { 56.0, 3.0 }));
...
{
    std::unique_ptr<model::FigureInterface> t(std::make_unique<model::Triangle>(
        Point { 1.0, 1.0 }, Point { 35.5, 1.0 }, Point { 35.6, 8.8 }));
